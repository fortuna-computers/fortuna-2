...
private:
    ...
    static std::vector<SourceFile> create_source_list(ProjectFile const& pf);
    static std::string             compiler_full_path();
    static std::string             execute_compiler(std::string const& path, std::string const& filename);
    static void                    load_binary(std::string const& path, std::string const& file, SourceFile const& source, CompilationResult& result);
    static Filenames               find_filenames(std::string const& path, CompilationResult& result);
    static void                    load_listing(std::string const& path, SourceFile const& source, Filenames const& filenames, CompilationResult& result);
    ...
    static size_t                  read_address(SourceFile const& source, Filenames const& filenames, std::string const& line, CompilationResult& result, size_t file_number, size_t file_line);
